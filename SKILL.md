...
图片理解和联网搜索依赖 `mcporter`。如果尚未安装，按以下步骤配置：
...
**2. 配置 MiniMax MCP 工具：**
在 `~/.openclaw/workspace/config/mcporter.json`（或全局 `~/.mcporter/mcporter.json`）中写入：
```json
{
  "mcpServers": {
    "MiniMax": {
      "command": "npx",
      "args": ["-y", "@modelcontextprotocol/server-minimax"],
      "env": {
        "MINIMAX_API_KEY": "你的 sk-cp- Key",
